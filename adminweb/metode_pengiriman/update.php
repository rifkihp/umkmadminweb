<?php
    ...
    $data = $db->getFieldValue("ekspedisi", array("gambar"), array("id" => $_POST["id"]));
    if(file_exists(realpath($root)."/uploads/ekspedisi/".$data[0]["gambar"])) {
        unlink(realpath($root)."/uploads/ekspedisi/".$data[0]["gambar"]);
    }
    
    //update data ekspedisi transfer
    $db->mysql_update("ekspedisi", $_POST, array("id" => $_POST["id"]));
    
    $_SESSION["alert_success"] = "Edit ekspedisi berhasil.";
    $db->mysql_close();
