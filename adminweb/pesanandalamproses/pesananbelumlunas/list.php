<?php
    ...
    $totalrecord = $db->getFieldValue("customer_order_header AS A LEFT JOIN member AS C ON A.id_member=C.id", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " A.status IN ('0', '1', '3') " . ($is_dropship!="0"?" AND A.is_dropship='$is_dropship'":"") . 
    ...
    $listdetail = $db->getFieldValue("customer_order_header AS A LEFT JOIN member AS C ON A.id_member=C.id", array("A.id", "A.kode AS no_trx",  "A.id_member", " C.email AS email_member", " CONCAT(C.first_name", " ' '", " C.last_name) AS nama_member", " DATE_FORMAT(A.tanggal_jam", " '%d-%m-%Y') AS tanggal_pesan", "A.nama", "A.total", "A.is_dropship", "A.pembayaran"), array(), array(), $limit, $limit*($currentpage-1), " A.id DESC ",  " A.status IN ('0', '1', '3') " . ($is_dropship!="0"?" AND A.is_dropship='$is_dropship'":"") . 
        (strlen($tanggal_dari)>0?" AND DATE_FORMAT(A.tanggal_jam, '%Y-%m-%d')>='$tanggal_dari' ":"") .
        (strlen($tanggal_sampai)>0?" AND DATE_FORMAT(A.tanggal_jam, '%Y-%m-%d')<='$tanggal_sampai' ":"") .
        (strlen($query)>0?" AND (CONCAT(C.first_name, ' ', C.last_name) LIKE '%$query%' OR A.kode LIKE '%$query%' OR A.nama LIKE '%$query%')":""));
    ...
    $smarty->assign("sub_page_selected", "pesananbelumlunas");    
    $smarty->assign("title", "Pesanan Belum Lunas");
    $smarty->assign("sub_title", "Daftar Pesanan Belum Lunas");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/pesanandalamproses/list_pesananbelumlunas.html");    