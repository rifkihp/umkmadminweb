<?php
    ...
    $totalrecord = $db->getFieldValue("customer_order_header AS A LEFT JOIN member AS C ON A.id_member=C.id", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " A.status=5 " . ($is_dropship!="0"?" AND A.is_dropship='$is_dropship'":"") . 
    ...
    $smarty->assign("sub_page_selected", "pesananselesai");    
    $smarty->assign("title", "Pesanan Telah Selesai");
    $smarty->assign("sub_title", "Daftar Pesanan Telah Selesai");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/pesanandalamproses/list_pesananselesai.html");    