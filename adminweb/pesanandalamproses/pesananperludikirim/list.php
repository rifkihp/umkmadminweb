<?php
    ...
    $totalrecord = $db->getFieldValue("customer_order_header AS A LEFT JOIN member AS C ON A.id_member=C.id", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " A.status=2 " . ($is_dropship!="0"?" AND A.is_dropship='$is_dropship'":"") . 
    ...
    $smarty->assign("sub_page_selected", "pesananperludikirim");    
    $smarty->assign("title", "Pesanan Perlu Dikirim");
    $smarty->assign("sub_title", "Daftar Pesanan Perlu Dikirim");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/pesanandalamproses/list_pesananperludikirim.html");    