<?php
    ...
    $listhalaman = $db->getFieldValue("menu_page AS A LEFT JOIN posisi_menu AS B ON A.posisi=B.id_posisi", array("A.id", "A.judul", "A.konten", "B.nama_posisi AS posisi", "A.is_aktif")); 
    foreach ($listhalaman as $key => $value) {
        $listhalaman[$key]["konten"] = $db->limitTextKonten($listhalaman[$key]["konten"], 150);        
    }
    
    $smarty->assign("listhalaman", $listhalaman);
    ...
    $tab = isset($_GET["tab"])?$_GET["tab"]:"halamanweb";
    ...
    $smarty->assign("sub_title", "Pengaturan Website");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/pengaturan_website/list.html");    