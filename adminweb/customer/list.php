...
$totalrecord = $db->getFieldValue("member AS A LEFT JOIN customer_address AS B ON B.id_member=A.id AND B.as_default=1", array("COUNT(*) AS TOTAL"), array("A.tipe" => 0),  array(), 0, 0, "", " 1 " . (strlen($query)>0?" AND (A.first_name LIKE '%$query%' OR A.last_name LIKE '%$query%' OR A.email LIKE '%$query%' OR B.alamat LIKE '%$query%' OR A.phone LIKE '%$query%')":""));
...
$smarty->assign("sub_title", "List of Pelanggan");
