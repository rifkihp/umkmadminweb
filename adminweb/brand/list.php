<?php
    ...
    $totalrecord = $db->getFieldValue("brand AS A", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " 1 " . (strlen($query)>0?" AND (A.kode LIKE '%$query%' OR A.nama LIKE '%$query%')":""));
    ...
    $listdetail = $db->getFieldValue("brand AS A", array("A.id", "A.kode", "A.nama", "A.logo"), array(), array(), $limit, $limit*($currentpage-1), " id DESC ",  " 1 " . (strlen($query)>0?" AND (A.kode LIKE '%$query%' OR A.nama LIKE '%$query%')":"")); 
    ...
    $smarty->assign("sub_title", "List of Brand");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/brand/list.html");    